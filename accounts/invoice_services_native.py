...
class StripeNativeInvoiceService:
    ...
    def ensure_customer(self, shopper: Shopper) -> str:
        ...
        try:
            ...
            customer = self.stripe.Customer.create(
                email=shopper.email,
                name=f"{shopper.first_name} {shopper.last_name}".strip()
                or shopper.username,
    ...
    def create_subscription(self, template_data: dict) -> dict:
        """
        Crée un abonnement récurrent avec Stripe Subscriptions
        """
        try:
            customer_id = self.ensure_customer(template_data["customer"])
            ...
            price = self.stripe.Price.create(
                unit_amount=int(template_data["amount"] * 100),
                currency=template_data.get("currency", "eur"),
                recurring={
                    "interval": interval_mapping.get(
                        template_data["frequency"], "month"
                    ),
                    "interval_count": 1,
                },
                product=product.id,
            ...
            logger.error(f"Erreur création abonnement Stripe: {e}")
            return {"success": False, "error": str(e)}
    ...
    def _create_payment_record(self, invoice: Invoice, stripe_invoice):
        ...
        try:
            ...
            InvoicePayment.objects.create(
                ...
                provider="stripe",
                provider_payment_id=stripe_invoice.payment_intent or stripe_invoice.id,
                status="completed",
...
class UnifiedInvoiceManager:
    ...
    def create_subscription(
        self, template_data: dict, provider: str = "stripe"
    ) -> dict:
        """
        Crée un abonnement récurrent
        """
        if provider == "stripe":
            return self.stripe_service.create_subscription(template_data)
        else:
            return {
                "success": False,
                "error": f"Abonnements non supportés pour {provider}",
            }
