    ...
    try:
        ...
        if result["success"]:
            ...
            if payment_intent.status == "requires_action":
                ...
                error_msg = f"Statut inattendu: {payment_intent.status}"
...
def payment_success(request):
    """Page de confirmation de paiement réussi avec notification"""
    ...
    context = {
        ...
        "redirect_url": "/accounts/order-history/",
    }
    ...
    if event["type"] == "payment_intent.succeeded":
        ...
        try:
            ...
            if transaction.order_id:
                ...
                # Fallback : toutes les commandes non payées
                cart_orders = Order.objects.filter(user=transaction.user, ordered=False)

            if cart_orders.exists():
                # Utiliser une transaction atomique pour la cohérence
                with db_transaction.atomic():
                    # Marquer les commandes comme payées
                    for order in cart_orders:
                        order.ordered = True
